...
-- ***** your code below *****
FROM events
RIGHT JOIN mortality mortal
ON (events.patient_id = mortal.patient_id);
...
-- ***** your code below *****
FROM events
LEFT JOIN mortality mortal
ON (events.patient_id = mortal.patient_id)
WHERE mortal.patient_id IS NULL;
...
-- ***** your code below *****
FROM (SELECT COUNT(patient_id) AS event_count FROM alive_events GROUP BY patient_id) reqd;
...
SELECT avg(event_count), min(event_count), max(event_count)
-- ***** your code below *****
FROM (SELECT COUNT(patient_id) AS event_count FROM dead_events GROUP BY patient_id) reqd;
...
-- ***** your code below *****
FROM (SELECT COUNT(DISTINCT(time)) AS encounter_count FROM alive_events GROUP BY patient_id) reqd;
...
SELECT avg(encounter_count), min(encounter_count), max(encounter_count)
-- ***** your code below *****
FROM (SELECT COUNT(DISTINCT(time)) AS encounter_count FROM dead_events GROUP BY patient_id) reqd;
...
-- ***** your code below *****
FROM (SELECT datediff(max(time), min(time)) as record_length FROM alive_events GROUP BY patient_id) reqd;
...
SELECT avg(record_length), percentile(record_length, 0.5), min(record_length), max(record_length)
-- ***** your code below *****
FROM (SELECT datediff(max(time), min(time)) as record_length FROM dead_events GROUP BY patient_id) reqd;
